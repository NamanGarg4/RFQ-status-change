...
class RequestforQuotation(BuyingController):
	...
	def update_status_on_sq_submission(self):
		if self.status in ["Submitted", "Quotation Partially Received"]:
			# Get all suppliers in this RFQ
			...
			# Determine new status based on response count
	...
	# Add this method inside the Request for Quotation class
	@frappe.whitelist()
	def update_to_quotation_requested(self):
		"""Update status to 'Quotation Requested' when 'Email It' button is clicked"""
		if self.docstatus == 1:  # Only update if document is submitted
			self.db_set("status", "Quotation Requested")
